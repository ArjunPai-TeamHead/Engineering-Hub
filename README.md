...
**Live website:** https://enginexus-hub.netlify.app/
**Preview (latest build):** https://enginexus-hub.netlify.app/
---
...
Supabase Cloud auto-provisions a Supabase project; you do not need a Supabase account. The frontend reads its connection from `.env` (auto-generated, do **not** edit):
...
1. Visit *https://enginexus-hub.netlify.app/**.
...
The site is a single-page app — refreshing any deep link works thanks to `public/_redirects` and Supabase's SPA fallback.
...
The project is hosted on netlify hosting (and works on Netlify via `netlify.toml`).
