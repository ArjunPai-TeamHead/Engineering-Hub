  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        ...
        data.forEach((d) => {
          if (!map[d.course_id]) map[d.course_id] = new Set();
          map[d.course_id].add(d.lesson_id);
        });
        const result: Record<string, number> = {};
        for (const [courseId, lessons] of Object.entries(map)) {
          const course = courses.find((c) => c.id === courseId);
          if (course) {
            result[courseId] = Math.round((lessons.size / course.lessons.length) * 100);
          }
        }
                    ...
                    {path.courses.map((cId, j) => (
                      <div key={cId} className="flex items-center gap-1">
                        <div className="h-3 w-3 rounded-full border-2" style={{
                          ...
                          backgroundColor: (progressMap[cId] ?? 0) === 100 ? path.color : "transparent",
                        }} />
